...
/**
 * @module formatElapse
 * @description Creates a nice string that shows the time since the passed in date
 * @memberof Vanllia
 * @version 1.0.1
...
export function formatElapse(dateParam) {
	...
	if (isToday) return formatDate('Today at {hh}:{mm}', date) // Today at 10:20
	if (isYesterday) return formatDate('Yesterday at {hh}:{mm}', date) // Yesterday at 10:20
